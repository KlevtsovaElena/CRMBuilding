...
<?php include('./../components/header.php'); ?>   
    ...
    ?>
        ...
        <section class="orders">
            <table id="list-orders" data-section="admin-wholesalers"  data-limit="<?php if (isset($_GET['limit'])) {?><?=$limit?><?php } else { ?><?=$limit?><?php } ?>" <?php if (isset($_GET['page'])) { ?> data-page="<?= $_GET['page'] ?>" <?php } else if (isset($_GET['search'])) { ?> data-search="<?= $_GET['search'] ?>" <?php } ?> data-city-select="<?php if (isset($_GET['city_name'])) { echo $_GET['city_name']; } ?>">
                ...
                <?php if (!isset($_GET['orderby'])) { 
                    ...
                    if(isset($_GET['page']) && $_GET['page'] > 1) {
                        ...
                        $dataJsonN = file_get_contents($nginxUrl . '/api/vendors/get-with-details.php?deleted=0&city_deleted=0&role=3'. $params);
                    ...
                    //если мы на первой странице
                        ...
                        if (!isset($_GET['search'])) {
                            //соберём данные для отображения в форме 
                            $dataJson = file_get_contents($nginxUrl . '/api/vendors/get-with-details.php?deleted=0&city_deleted=0&role=3&limit=' . $limit .  $params);
                            $data = json_decode($dataJson, true);
                ...
                elseif (isset($_GET['orderby'])) {
                    ...
                    if(isset($_GET['page']) && $_GET['page'] > 1) {
                        ...
                        //отдельно соберем информацию об общем кол-ве записей
                        $dataJsonN = file_get_contents($nginxUrl . '/api/vendors/get-with-details.php?deleted=0&city_deleted=0&role=3&orderby=' . $_GET['orderby'] . $params);
                        $dataN = json_decode($dataJsonN, true);
                        $totalNumElements = count($dataN);
                        //и об общем кол-ве страниц
                        $totalPages = ceil(count($dataN) / $limit);
                    ...
                    //если мы на первой странице
                        ...
                        if (!isset($_GET['search'])) {
                            //соберём данные для отображения в форме 
                            $dataJson = file_get_contents($nginxUrl . "/api/vendors/get-with-details.php?deleted=0&city_deleted=0&role=3&limit=" . $limit . '&orderby=' . $_GET['orderby'] . $params);
                            $data = json_decode($dataJson, true);
                            $num = 1; //переменная для отображения порядкового номера (чтобы не было пропусков, т.к. некоторые id "удалены")
