...
<?php 
        if (count($_GET) == 0)  {
            ...
            $ordersJson = file_get_contents($url . "&offset=0&limit=" . $limit);
        ...
        <tbody class="list-orders__body">
            <?php foreach ($orders['orders'] as $order) { 
                ...
                for ($i = 0; $i < count($order['products']); $i++) {
                    ...
                    $totalPrice += $order['products'][$i]['quantity'] * $order['products'][$i]['price'];
                }
                echo $totalPrice;
            ...
            <tr role="row" class="list-orders__row row-status<?= $order['status']; ?>" order-id="<?= $order['id']; ?>">
                ...
                <td><a href="./../pages/vendor-order.php?id=<?= $order['id']; ?>"><strong><?= $order['status']; ?></strong></a></td>
                <td><?= $date; ?></td>
                <td><a href="./../pages/vendor-order.php?id=<?= $order['id']; ?>" class="list-orders_status d-block status${status}"><?= $order['status']; ?></a></td>
